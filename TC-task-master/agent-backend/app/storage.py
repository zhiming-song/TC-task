...
class TravelRepository:
    ...
    def _seed_mock_catalog(self, connection: sqlite3.Connection) -> None:
            ...
            for destination_index, destination in enumerate(city_names):
                ...
                for transport_type, suffix, departure_time, duration, price, seat_class, inventory in specs:
                    ...
                    if transport_type == "train":
                        service_no = f"G{201 + route_score % 700} 次"
                    else:
                        service_no = f"CA{1201 + route_score % 8600} 次"
        ...
        real_shanghai_hotels = (
            ("mock_hotel_shanghai_jiquan_jiangsu", "全季酒店（上海中山公园江苏路店）", "中山公园",
            ...
            ("mock_hotel_shanghai_hyatt_place_hongqiao", "上海虹桥商务区凯悦嘉轩酒店", "虹桥火车站",
             "comfort", "4.6", 49300, 6,
             _json(["步行10分钟进虹桥站", "外滩2号线直达约35分钟", "35平双床房"]),
             "https://images.unsplash.com/photo-1590073242678-70ee3fc28e8e?w=400&q=60"),
            ("mock_hotel_shanghai_cordis_hongqiao", "上海虹桥康得思酒店", "虹桥火车站",
             "comfort", "4.8", 69900, 5,
             _json(["地下连廊直通高铁站", "豪华型41平客房", "带泳池"]),
             "https://images.unsplash.com/photo-1582719508461-905c673771fd?w=400&q=60"),
        )
        shanghai_ticket_images = {
            "上海迪士尼乐园": "https://images.unsplash.com/photo-1513889961551-628c1e5e2ee9?w=400&q=60",
            "东方明珠": "https://images.unsplash.com/photo-1548919973-5cef591cdbc9?w=400&q=60",
            "上海博物馆": "https://images.unsplash.com/photo-1554907984-15263bfd63bd?w=400&q=60",
            "上海城市观光": "https://images.unsplash.com/photo-1474181487882-5abf3f0ba6c2?w=400&q=60",
        }
            ...
            for attraction_index, attraction in enumerate(attractions):
                ...
                ticket_rows.append(
                    (
                        ...
                        "09:00-18:00（以景区当日公告为准）",
                        settings.tongcheng_ticket_booking_url,
                        _json([attraction, "可退规则待确认"]),
                        shanghai_ticket_images.get(attraction, ""),
                        timestamp,
                    )
                )
        connection.executemany(
