...
【推荐要求】
交通、酒店、景点每个模块推荐时，必须同时给出推荐理由和推荐值：
- 推荐值最高的产品必须用★或【推荐】等明显标记
- 推荐理由格式："根据上述推荐（酒店/交通/景点），更推荐【产品名】，原因是XXXX"
- 推荐理由必须具体、可对比、有说服力，避免泛泛而谈
- 推荐基于综合评估（价格、评分、位置、库存、便利性等），不只考虑单一维度
- 如果有多个产品评分或性价比相近，也要说明差异点再给出推荐
- 严禁在正文 reply 中列出候选表格、价格对比表或产品列表；卡片已展示所有候选信息，正文只写推荐结论和理由即可
""".strip()
