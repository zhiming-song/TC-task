...
def search_transport(args: dict[str, Any]) -> dict[str, Any]:
    ...
    if not inventory_rows:
        raise TravelToolError(f"商品库暂无{origin}到{destination}且库存满足{travelers}人的交通产品")
        ...
        candidates.append(
            {
                ...
                "preference_notes": "商品来自同程商品库；班次、价格和余票以下单时为准。",
            }
        )
    response = {
        **_demo_notice(),
...
def search_hotels(args: dict[str, Any]) -> dict[str, Any]:
    ...
    if not inventory_rows:
        raise TravelToolError(f"商品库暂无{destination}且库存满足{rooms}间房的酒店产品")
    if locations:
        matched_rows = [
            row
            for row in inventory_rows
...
def search_attractions(args: dict[str, Any]) -> dict[str, Any]:
    ...
    if not inventory_rows:
        raise TravelToolError(f"商品库暂无{destination}且库存满足{travelers}人的门票产品")
    results = []
    for row in inventory_rows:
        unit_cents = int(row["unit_price_cents"])
        remaining = int(row["remaining_inventory"])
